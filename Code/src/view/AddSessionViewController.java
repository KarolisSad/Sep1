...
public class AddSessionViewController
{
  ...
  @FXML private void check()
  {
    ...
    DateTime dateTime = new DateTime(new Date(day, month, year),
        new Time(Integer.parseInt(hour.getText()),
            Integer.parseInt(minute.getText())));
    DateTime endDate = new DateTime(new Date(day, month, year), new Time(
        (Integer.parseInt(hour.getText())) + (Integer.parseInt(
            length.getText())), Integer.parseInt(minute.getText())));
    ...
    for (int i = 0; i < rooms.getNumberOfRooms(); i++)
    {
      roomList.getItems().add(
          rooms.getRoom(i).getRoomNumber() + " Capacity: " + rooms.getRoom(i)
              .getCapacity());
    }
  }
  ...
  @FXML private void next()
  {
    ...
    try
    {
      ...
      String selectedRoom[] = theRoom.split(" ");
      theRoom = selectedRoom[0];

     model.addSession(course.getText(), Integer.parseInt(length.getText()),
       day, month, year, Integer.parseInt(hour.getText()),
       Integer.parseInt(minute.getText()), theRoom);
