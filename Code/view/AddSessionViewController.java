...
public class AddSessionViewController
{
  ...
  public void init(ViewHandler viewHandler, Region root, ScheduleModel model)
  {
    ...
    length.setPromptText("e.g. 2");
    hour.setPromptText("e.g. 8");
    minute.setPromptText("e.g. 20");
  }
  ...
  /**
   * checks for avaliable rooms within selected time frame
   * and adds to choice box
   */
  @FXML private void check()
  {
    ...
    {
      roomList.getItems().add(rooms.getRoom(i).getRoomNumber()+ " Capacity: " + rooms.getRoom(i).getCapacity());
    }
  }
  ...
  @FXML private void next()
  {
    ...
    model.addSession(course.getText(),Integer.parseInt(length.getText()),day,month,year,Integer.parseInt(hour.getText()),Integer.parseInt(minute.getText()),theRoom);
      System.out.println("Test - 1 " + model.getSession(0).getCourse().getCourseStudentList().size());
    error.setText("Successfully added");
      System.out.println(model.getCourse(0).getCourseName());
